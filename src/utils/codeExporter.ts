...
import type { AgentFlowNode, AgentFlowNodeType, LLMModel } from '../types'
...
const MODEL_SETUP: Record<LLMModel, (varName: string, temp: number) => string> = {
  'gemini-flash': (v, t) =>
    `${v} = ChatGoogleGenerativeAI(model="gemini-1.5-flash", temperature=${t})`,
  'gemini-pro': (v, t) =>
    `${v} = ChatGoogleGenerativeAI(model="gemini-1.5-pro", temperature=${t})`,
 ...
 */
function spliceOutNodes(edges: Edge[], removeIds: string[]): Edge[] {
  let result = [...edges]
...
export function exportRequirements(nodes: AgentFlowNode[]): string {
  ...
  if (hasOllama) lines.push('langchain-community')
...
export function exportPython(
  ...
  if (hasOllama) emit('from langchain_community.chat_models import ChatOllama')
